...
import {
  LayoutDashboard, User, Compass, CalendarCheck, Briefcase,
  MessageCircle, LogOut, ChevronRight, Zap, Bell, Settings, X,
...
function RoleToggle({ role, setRole, collapsed = false }) {
  return (
    <div
      ...
      title={collapsed ? `Switch to ${role === "student" ? "Tutor" : "Student"}` : undefined}
    >
      {collapsed ? (
        ...
        ["student", "tutor"].map(r => (
          ...
          >
            ...
            <span className="relative z-10 flex items-center gap-1">
              {r === "student" ? <GraduationCap size={10} /> : <BookOpen size={10} />}
...
function DesktopSidebar({ profile, pathname, onLogout, unreadCount, role, setRole }) {
  ...
  const visibleItems = NAV_ITEMS.filter(item => !(item.studentOnly && role === "tutor"));
  ...
  return (
    ...
    >
      {/* Logo row */}
      <div className="flex h-14 items-center px-4" style={{ borderBottom: "1px solid #1a1814" }}>
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.span
                ...
                className="text-sm font-medium truncate" style={{ color: "#f5f0e8" }}>
                SkillBridge
              </motion.span>
            )}
          </AnimatePresence>
          ...
          return (
            <Link key={href} href={href}>
              <motion.div whileTap={{ scale: 0.97 }}
                ...
                style={{ background: active ? "rgba(232,184,75,0.08)" : "transparent", color: active ? "#e8b84b" : "#6a6050" }}
                onMouseEnter={e => { if (!active) { e.currentTarget.style.background = "rgba(255,255,255,0.03)"; e.currentTarget.style.color = "#c8bfb0"; } }}
                onMouseLeave={e => { if (!active) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#6a6050"; } }}>
                {active && (
        ...
        <AnimatePresence mode="wait">
          {!collapsed ? (
            <motion.div key="expanded"
              ...
              {/* Role label */}
              <p className="mb-1.5 px-1 text-[10px] font-medium uppercase tracking-widest" style={{ color: "#3a342c" }}>
                Role
              </p>
      ...
      <div className="relative px-2 pb-3" ref={menuRef}>
        <motion.button whileTap={{ scale: 0.97 }}
          ...
          {profile?.avatar_url ? (
            <img src={profile.avatar_url} alt={profile.name} className="h-7 w-7 shrink-0 rounded-lg object-cover" />
          ) : (
            <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-medium"
            ...
            {!collapsed && (
              <motion.div initial={{ opacity: 0, x: -4 }} animate={{ opacity: 1, x: 0 }}
                ...
                <p className="truncate text-xs" style={{ color: "#4a4438" }}>
                  {role === "tutor" ? "Tutor mode" : profile?.department?.split("·")[0]?.trim() || "Campus"}
                </p>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            <motion.div
              ...
              {[
                ...
                <Link key={href} href={href} onClick={() => setUserMenuOpen(false)}>
                  <div className="flex items-center gap-2.5 px-3 py-2.5 text-xs transition-colors"
                    style={{ color: "#8a8070" }}
                    onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,0.03)"; e.currentTarget.style.color = "#c8bfb0"; }}
...
function MobileBottomNav({ pathname, unreadCount, role }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 flex items-center px-2 pb-safe"
      style={{ background: "rgba(10,9,8,0.96)", backdropFilter: "blur(16px)", borderTop: "1px solid #1a1814", height: 60 }}>
      {mobileItems.map(({ href, icon: Icon, label }) => {
        ...
        return (
          <Link key={href} href={href} className="flex flex-1 flex-col items-center justify-center gap-0.5 py-1">
            <motion.div whileTap={{ scale: 0.88 }} className="relative flex flex-col items-center gap-0.5">
              <div className="flex h-8 w-8 items-center justify-center rounded-xl transition-all"
                style={{ background: active ? "rgba(232,184,75,0.12)" : "transparent" }}>
                <Icon size={18} style={{ color: active ? "#e8b84b" : "#4a4438" }} />
                {isChatWithBadge && (
                  <span className="absolute -right-0.5 -top-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full"
                    style={{ background: "#e8b84b", color: "#0e0c0a", fontSize: 8, fontWeight: 600 }}>
                    {unreadCount > 9 ? "9+" : unreadCount}
                  </span>
                )}
              </div>
              <span className="text-xs" style={{ color: active ? "#e8b84b" : "#3a342c", fontSize: 10 }}>{label}</span>
              {active && (
...
function MobileTopBar({ profile, pathname, onLogout, unreadCount, role, setRole }) {
  ...
  return (
    <>
      <header className="fixed left-0 right-0 top-0 z-40 flex h-14 items-center justify-between px-4"
        style={{ background: "rgba(10,9,8,0.96)", backdropFilter: "blur(16px)", borderBottom: "1px solid #1a1814" }}>
        <Link href="/dashboard" className="flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg"
            style={{ background: "rgba(232,184,75,0.12)", border: "1px solid rgba(232,184,75,0.18)" }}>
        ...
        <div className="flex items-center gap-2">
          {/* Compact role toggle in top bar */}
          <div className="flex items-center gap-0.5 rounded-lg p-0.5"
            style={{ background: "#141210", border: "1px solid #2a2520" }}>
            {["student", "tutor"].map(r => (
              <button key={r} onClick={() => setRole(r)}
                className="relative rounded-md px-2 py-1 text-[10px] font-medium capitalize transition-colors"
                style={{ color: role === r ? "#0e0c0a" : "#6a6050" }}>
                {role === r && (
                  <motion.div layoutId="topbar-role-pill"
                    className="absolute inset-0 rounded-md"
                    style={{ background: "#e8b84b" }}
                    transition={{ type: "spring", bounce: 0.2, duration: 0.38 }} />
                )}
                <span className="relative z-10">{r === "student" ? "Student" : "Tutor"}</span>
              </button>
            ))}
          </div>

          <motion.button whileTap={{ scale: 0.93 }} onClick={() => setMenuOpen(true)}
            className="flex h-8 w-8 items-center justify-center rounded-lg"
            style={{ background: "rgba(232,184,75,0.08)" }}>
            {profile?.avatar_url ? (
              <img src={profile.avatar_url} alt="" className="h-8 w-8 rounded-lg object-cover" />
            ) : (
              <span className="text-xs font-medium" style={{ color: "#e8b84b" }}>{initials}</span>
            )}
          </motion.button>
        </div>
      </header>
        ...
        {menuOpen && (
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
              ...
              <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid #1a1814" }}>
                <div className="flex items-center gap-3">
                  {profile?.avatar_url ? (
                    ...
                    <div className="flex h-9 w-9 items-center justify-center rounded-xl text-sm font-medium"
                      style={{ background: "rgba(232,184,75,0.12)", color: "#e8b84b" }}>
                      {initials}
                    </div>
                  )}
                ...
                <motion.button whileTap={{ scale: 0.93 }} onClick={() => setMenuOpen(false)}
              ...
              <div className="px-4 py-3" style={{ borderBottom: "1px solid #1a1814" }}>
                <p className="mb-2 text-[10px] font-medium uppercase tracking-widest" style={{ color: "#3a342c" }}>
                  Viewing as
                </p>
                <RoleToggle role={role} setRole={(r) => { setRole(r); }} collapsed={false} />
              </div>
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0e0c0a" }}>
      {isMobile ? (
        <>
          <MobileTopBar
            profile={profile} pathname={pathname}
            onLogout={handleLogout} unreadCount={unreadCount}
            role={role} setRole={setRole}
          />
          <main className="pb-16 pt-14">{children}</main>
          <MobileBottomNav pathname={pathname} unreadCount={unreadCount} role={role} />
        </>
      ) : (
        <div className="flex">
          <DesktopSidebar
            ...
            role={role} setRole={setRole}
          />
          <main className="flex-1 transition-all duration-300" style={{ marginLeft: 220 }}>
            {children}
          </main>
        </div>
      )}
